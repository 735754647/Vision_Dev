...
#define HVGIGE_WP_API extern"C" __declspec(dllexport)
#else
#define HVGIGE_WP_API extern"C" __declspec(dllimport)
#endif
...
/******************************************************************************
* Function:	HV_GetTestInfo
* Description: 获取测试参数
* Format:
*     HRESULT HV_GetTestInfo(int & nImgCount,int & nPacketMissing, int nDevId) ;
* Params:
*     int & nImgCount: 图像总帧数
...
*     错误字符串
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_GetTestInfo(int & nImgCount,int & nPacketMissing,int &nFrameMissing, int nDevId);
...
* Format:
*     HRESULT HV_SetSnapDebounceTime(LONG lDebouncerTime,int nDevId)
* Params:
...
HVGIGE_WP_API HRESULT __stdcall HV_SetSnapDebounceTime(LONG lDebouncerTime,int nDevId);
...
* Format:
*     HV_SetSnapExpTime(LONG lExpTime,int nDevId)
* Params:
...
HVGIGE_WP_API HRESULT __stdcall HV_SetSnapExpTime(LONG lExpTime,int nDevId);
...
HVGIGE_WP_API HRESULT __stdcall HV_SetSnapTrigFreqImgCount(LONG lTrigFreq, LONG lTrigImgCount,int nDevId);

/******************************************************************************
* Function:	HV_SetSnapTrigFreqImgCount_USecond()
* Description: 设置拍照模式时，触发的间隔时间以及每次触发输出的图像张数
* Format:
*     HRESULT HV_SetSnapTrigFreqImgCount_USecond(LONG lTrigFreq, LONG lTrigImgCount,int nDevId)
* Params:
*	 LONG lTrigTime: 设置拍照模式时，图像输出时间间隔。单位为微秒。
*    LONG lTrigImgCount：触发一次，连续输出的图像帧数
* Return:
*	   HV_SUCCESS: 成功设置Exposure的值；
*      HV_NO_SUCH_DEVICE: 没有找到对应的硬件设备
...
HVGIGE_WP_API HRESULT __stdcall HV_SetSnapTrigFreqImgCount_USecond(LONG lTrigTime, LONG lTrigImgCount,int nDevId);
...
HVGIGE_WP_API HRESULT __stdcall HV_GetSnapTrigFreqImgCount(LONG* plTrigFreq, LONG* plTrigImgCount,int nDevId);
...
* Format:
*     HRESULT HV_SaveParaToIniFile(int nDevId)
* Params:
*	  none
* Return: 
*     void
...
HVGIGE_WP_API HRESULT __stdcall HV_SaveParaToIniFile(int nDevId);
...
* Format:
*      HRESULT HV_GetROI(int Original_x, int Original_y,
*						int nImgWidth, int nImgHeight,int nDevId)
* Params:
...
*      HV_NO_SUCH_DEVICE: 没有找到对应的硬件设备
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_GetROI(int &nStart_x, int &nStart_y, 
										  int &nImgWidth, int &nImgHeight,int nDevId);
...
/******************************************************************************
* Function:	HV_SetPacketSize
* Description: 设置sGain.nValue[0:4]的值
* Format:
*     HRESULT HV_SetPacketSize(int nPacketSize,int nDevId)
* Params:
*	   int nPacketSize   设置包大小
*	   int nDevId   设备索引
* Return:
*      HV_DEVICE_CONNECT_TIMEOUT:链接超时；
*	   HV_SUCCESS: 成功设置包大小；   
******************************************************************************/

HVGIGE_WP_API HRESULT __stdcall HV_SetPacketSize(int nPacketSize,int nDevId);

/******************************************************************************
* Function:	HV_SetGrrMode_ExpFlash()
* Description: 设置Drr模式下的曝光时间与控制闪光灯灭的延迟时间
* Format:
*     HRESULT HV_SetGrrMode_ExpFlash(long lExposureTime, long lFlashDelayTime, int nDevId)
* Params:
*     long lExposureTime : Grr 模式下的曝光时间 （单位us）
*     long lFlashDelayTime ： 闪光灯灭的延时时间（单位us）
*	  int nDevId : 相机索引号
* Return: 
*     HV_SUCCESS: 成功   HV_UNSUCCESS：不成功，没有设置Grr模式
******************************************************************************/

HVGIGE_WP_API HRESULT __stdcall HV_SetGrrMode_ExpFlash(long lExposureTime, long lFlashDelayTime, int nDevId);

/******************************************************************************
* Function:	HV_GetGrrMode_ExpFlash()
* Description: 获取Drr模式下的曝光时间与控制闪光灯灭的延迟时间
* Format:
*     HRESULT HV_GetGrrMode_ExpFlash(long &lExposureTime, long &lFlashDelayTime, int nDevId)
* Params:
*     long lExposureTime : Grr 模式下的曝光时间 （单位us）
*     long lFlashDelayTime ： 闪光灯灭的延时时间（单位us）
*	  int nDevId : 相机索引号
...
*     HV_SUCCESS: 成功   HV_UNSUCCESS：不成功，没有设置Grr模式
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_GetGrrMode_ExpFlash(long &lExposureTime, long &lFlashDelayTime, int nDevId);
...
/******************************************************************************
* Function:	HV_SetImageDataType
* Description: 设置相机输出的图像格式
* Format:
*     HRESULT HV_SetImageDataMode(int nDevId) ;
* Params:
*     void
* Return: 
*     HV_SUCCESS: 成功
...
HVGIGE_WP_API HRESULT __stdcall HV_SetImageDataType(int nDevId, int nImgDataType);


/******************************************************************************
* Function:	HV_GetCameraType()
* Description: 得到当前相机型号(Camera Type),如：HV130GM等
* Format:
*     int HV_GetCameraType(int nDevId)
* Params:
*     int nDevId  设备索引
* Return: 
*     成功：返回Camera Type，否则返回0
******************************************************************************/
HVGIGE_WP_API CAMERA_TYPE __stdcall HV_GetCameraType(int nDevId);

/******************************************************************************
* Function:	HV_SetBrightnessFactor()
* Description: 设置相机亮度
* Format:
*     HV_SetBrightnessFactor(float Brightness, int nDevId)
* Params:
*	  float Brightness		亮度        有效范围(-1,1]  
*     int nDevId  设备索引
* Return: 
*     成功：return HV_SUCCESS; 若输入值超出范围则默认为-0.94和1
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_SetBrightnessFactor(float Brightness, int nDevId);

/******************************************************************************
* Function:	HV_GetBrightnessFactor()
* Description: 获取当前相机亮度值
* Format:
*     HV_GetBrightnessFactor(float &Brightness, int nDevId)
* Params:
*	  float &Brightness		亮度        有效范围(-1,1]  
*     int nDevId  设备索引
* Return: 
*     成功：return HV_SUCCESS; 
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_GetBrightnessFactor(float &Brightness, int nDevId);

/******************************************************************************
* Function:	HV_SetContrastFactor()
* Description: 设置相机对比度
* Format:
*     HV_SetContrastFactor(float Constrast, int nDevId)
* Params:
...
*     成功：return HV_SUCCESS; 若输入值超出范围则默认为-0.94和1
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_SetContrastFactor(float Constrast, int nDevId);

/******************************************************************************
* Function:	HV_GetContrastFactor()
* Description: 设置相机对比度
* Format:
*     HV_GetContrastFactor(float &Constrast, int nDevId)
* Params:
*	  float Constrast       对比度		有效范围(-1,1]
*     int nDevId  设备索引
* Return: 
*     成功：return HV_SUCCESS; 若输入值超出范围则默认为-0.94和1
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_GetContrastFactor(float &Constrast, int nDevId);

/******************************************************************************
* Function:	HV_AKeyWhiteBalance()
* Description: 一键白平衡
* Format:
*     HV_AKeyWhiteBalance(int nDevId)
* Params:
*     int nDevId  设备索引
* Return: 
*     成功：return HV_SUCCESS; 注：只针对彩色相机
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_AKeyWhiteBalance(int nDevId);

/******************************************************************************
* Function:	HV_SetUserDefNum()
* Description: 设置用户自定义序列号
* Format:
*     HRESULT HV_SetUserDefNum(LPSTR lpSeriesNum,int nDevId)
* Params:
...
*     HV_SUCCESS: 成功
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_SetUserDefNum(LPSTR lpSeriesNum, int nDevId);

/******************************************************************************
* Function:	HV_GetUserDefNum()
* Description: 获取用户自定义序列号
* Format:
*     HRESULT HV_GetUserDefNum(LPSTR lpSeriesNum,int nDevId)
* Params:
...
*     HV_SUCCESS: 成功
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_GetUserDefNum(LPSTR lpSeriesNum, int nDevId);

/******************************************************************************
* Function:	HV_SetGPIOMode(int nMode, int nDevId)
* Description: 选择GPIO输出类型
* Format:
*     HRESULT  HV_SetGPIOMode(int nMode, int nDevId)
* Params:
...
*     HV_SUCCESS: 成功
*     HV_UNSUCCESS: 失败
*     HV_INVALID_PARAM: 无效参数
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall	HV_SetGPIOMode(int nMode, int nDevId);

/******************************************************************************
* Function:	HV_SetGPIOValue(bool bValue, int nDevId)
* Description: 选择GPIO输出类型
* Format:
*     HRESULT  HV_SetGPIOValue(bool bValue, int nDevId)
* Params:
*     bool bValue   False为断开，True为接通，当HV_GPIOMode 选为闪光灯（值为0）模式时，该函数无效
*     int nDevId   相机索引号
* Return: 
*     HV_SUCCESS: 成功
*     HV_UNSUCCESS: 失败
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall	HV_SetGPIOValue(bool bValue, int nDevId);

/******************************************************************************
* Function:	HV_GetTriggerCount()
* Description: 获取触发计数
* Format:
*     HRESULT  HV_GetTriggerCount(int &nTrigCount, bool bReset, int nDevId)
* Params:
...
*     HV_SUCCESS: 成功
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall	HV_GetTriggerCount(int &nTrigCount, bool bReset, int nDevId);

/******************************************************************************
* Function:	HV_GetMaxFrameRate()
* Description: 获取当前相机能够输出的最大帧率
* Format:
*     HRESULT  HV_GetMaxFrameRate(int &nMaxFrameRate, int nDevId)
* Params:
*     int &nMaxFrameRate			获取当前相机最大帧率
*     int nDevId					相机索引号
* Return: 
*     HV_SUCCESS:					成功
*     HV_FEATURE_NOT_SUPPORT		不支持该特性
*	  HV_SEND_PACKET_FAIL			发送数据包失败
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall	HV_GetMaxFrameRate(int &nMaxFrameRate, int nDevId);


/******************************************************************************
* Function:	HV_GetFatorySeriesNum()
* Description: 获取相机出厂序列号
* Format:
*     HRESULT HV_GetFatorySeriesNum(LPSTR lpSeriesNum,int nDevId)
* Params:
*     LPCSTR lpSeriesNum : 返回用户设定序列号
*	  int nDevId : 相机索引号
* Return: 
...
HVGIGE_WP_API HRESULT __stdcall HV_GetFatorySeriesNum(LPSTR lpSeriesNum, int nDevId);
...
*      HV_NO_SUCH_DEVICE: 没有找到对应的硬件设备
******************************************************************************/
HVGIGE_WP_API HRESULT __stdcall HV_SetSpecialROI(int Offset_x1, int Offset_y1, int nWidth1, int nHeight1,
												int Offset_x2, int Offset_y2, int nWidth2, int nHeight2,int nDevId);
